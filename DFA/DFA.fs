...
type State<'a, 'char when 'char: equality> (id:'a, isStart:bool, isFinal:bool) =
    ...
    member this.AddTransition tupleOfChars state =
        if transitions.ContainsKey(tupleOfChars)
        then failwithf $"Transition for symbol {tupleOfChars} already exists!"
...
type Result<'a, 'char when 'char: equality> = Accept | Partial of list<'char> | Fail of State<'a, 'char>

[<Struct>]
type Configuration<'a, 'char when 'char: equality> =
    val CurrentState: State<'a, 'char>
    val RestOfInput: list<'char> 
    new (state, rest) = {CurrentState = state; RestOfInput = rest}
...
type DFA<'a, 'char when 'char: equality>(startState: State<'a, 'char>) =
    ...
    let rec step (configuration:Configuration<'a, 'char>) =
        ...
        | [] ->
            if configuration.CurrentState.IsFinal
            then Accept
            else Fail configuration.CurrentState
        ...
        let dfs (state:State<_,_>) =
            ...
            while toProcess.Count > 0 do
                ...
                then
                    ...
                    nodes.Add ($"{curNodeId} [fillcolor={nodeColor} shape={nodeShape} style=filled label=\"{state.Id}\"]")
                    addTransitions curNodeId
                    for (char, targetState) in  state.GetAllTransitions() do
                        toProcess.Push(targetState,(fun newTargetStateId ->
...
module DFA =
        ...
        let dfs (state:State<_,_>) =
            ...
            while toProcess.Count > 0 do
                ...
                then
                    ...
                    if newState.IsStart then newStartState <- newState
                    addTransitions newState
                    for (char, targetState) in  state.GetAllTransitions() do
    ...
    let intersection (dfa1:DFA<_,_>) (dfa2:DFA<_,_>) =
        let dfs (state1:State<_,_>) (state2:State<_,_>) =
            let mutable newStartState = Unchecked.defaultof<_>
            let toProcess = Stack<State<_,_>*State<_,_>*(State<_,_> -> unit)>[(state1, state2, fun _ -> ())]
            let copyOfState = Dictionary<_,_>()
            while toProcess.Count > 0 do
                ...
                if not exists
                then
                    let newState = State<_,_> ($"{state1.Id}_{state2.Id}" , state1.IsStart, state1.IsFinal && state2.IsFinal)
                    if newState.IsStart then newStartState <- newState
                    addTransitions newState
                    for (char1, targetState1) in  state1.GetAllTransitions() do
                        for (char2, targetState2) in state2.GetAllTransitions() do
                            if char1 = char2
                            then
                                toProcess.Push(targetState1, targetState2, (fun newTargetState -> newState.AddTransition char1 newTargetState))
